...
type CDNCheck struct {
	...
	CDNCheckResult CDNCheckResult
}

type CDNCheckResult struct {
	Domain  string
	IPs     common.StringSlice
	IsCDN   bool
	CDNName common.StringSlice
	Cname   common.StringSlice
}
...
func (c *CDNCheck) CheckASN(ip string) (isCDN bool) {
	...
	GeoLiteFile := filepath.Join(common.CurrentRunPath, common.GeoLite2Path)
...
func (c *CDNCheck) CheckCName(domain string) (cdnCheckResult CDNCheckResult) {
	var cdnNames = []string{}
		...
		if cname != domain {
			for _, cn := range cnames {
				if strings.Index(cname, cn) >= 0 {
					cdnNames = append(cdnNames, cn)
				}
			}
			cdnCheckResult = CDNCheckResult{
				Domain: domain,
				IsCDN:  true,
				//CDNName: strings.Join(cdnNames, "|"),
				CDNName: cdnNames,
				Cname:   []string{cname},
			}
		} else {
	...
	cdnCheckResult.Domain = domain
	return cdnCheckResult
}

func (c *CDNCheck) GetDomainHosts(domain string) (hosts []string) {
	//CName, _ = net.LookupCNAME(domain)
	host, _ := net.LookupHost(domain)
	for _, h := range host {
		if common.CheckIPV4(h) {
...
func (c *CDNCheck) RunCDNcheck(domain string) {
	cdnCheckResult := c.CheckCName(domain)
	hosts := c.GetDomainHosts(domain)
	//cdnCheckResult.IPs = strings.Join(hosts, "|")
	cdnCheckResult.IPs = hosts
	if cdnCheckResult.IsCDN == false {
		if len(hosts) <= 3 {
			for _, ip := range hosts {
				if (c.CheckIP(ip)) == true || (c.CheckASN(ip)) == true {
					cdnCheckResult.IsCDN = true
					cdnCheckResult.CDNName = []string{"cdn_ip"}
					break
				}
			}
		} else {
	...
	c.CDNCheckResult = cdnCheckResult
}
